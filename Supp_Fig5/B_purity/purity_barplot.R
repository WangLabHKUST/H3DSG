...
library(ggrepel)
library(ggbeeswarm)
NAN_plot <- ggplot(data=purity_data,
                   aes(x=final_dec,y=purity)) + theme_classic() +
  geom_boxplot(data=purity_data,
               aes(x=final_dec,y=purity),width = 0.6,size=0.5,fill="transparent",
               outlier.colour = "white")+#,color=final_dec
  geom_quasirandom(data=purity_data,aes(x=final_dec,y=purity, color=final_dec, 
                                                   fill=final_dec),
...
the<-compare_means(method = "wilcox.test",purity ~ final_dec,  
                   data = purity_data,paired = F )
NAN_plot <- NAN_plot +stat_pvalue_manual(
  the, label = NULL, size = 3,linetype = 1,
  y.position = c(0.96)
)
figure_2<-rbind(ggplotGrob(NAN_plot),size="last")
ggsave(file="purity_box_comparison_0320.pdf",
